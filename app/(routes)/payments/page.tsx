...
export default function PaymentsPage() {
  ...
  // 1. Intercept execution and return the global loader as long as the student hook is fetching
  if (isLoading) {
    return <GlobalLoader />;
  }
      ...
      <div className="container mx-auto py-10 pt-24 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500">
              Monitor student accounts, fees, and real-time payment status.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-slate-900">
                ...
                {students
                  ?.reduce(
                    (sum: number, s: any) => sum + (s.totalFees - s.balance),
                    0,
                  )
                  .toLocaleString()}
              </div>
          ...
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-slate-900">
                {students?.filter((s: any) => s.balance <= 0).length} /{" "}
                {students?.length}
              </div>
              <p className="text-xs text-slate-400 mt-1">
  ...
  return (
    <div className="container mx-auto py-10 pt-24 max-w-4xl space-y-8">
      <div>
        ...
        <p className="text-slate-500">
          View your current balance and make payments to settle your account.
        </p>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                <div className="p-4 rounded-xl bg-white border border-slate-100 shadow-sm">
                  ...
                  <p className="text-xl font-bold text-slate-900">
                    ${currentStudent.totalFees.toFixed(2)}
                  </p>
                </div>
                <div className="p-4 rounded-xl bg-white border border-slate-100 shadow-sm">
                  <p className="text-xs font-bold text-slate-400 uppercase mb-1">
                  ...
                  <p className="text-xl font-bold text-emerald-600">
                    $
                    {(
                      currentStudent.totalFees - currentStudent.balance
                    ).toFixed(2)}
                  </p>
                </div>
                <div className="p-4 rounded-xl bg-white border border-slate-100 shadow-sm">
                  ...
                  >
                    {currentStudent.balance <= 0
                      ? "SETTLED"
                      : currentStudent.isCriticalOverdue
                        ? "OVERDUE"
                        : "OUTSTANDING"}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-sm">
                      Instant update to your student record
                    </p>
                  </div>
                </div>
          ...
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent>
              <p className="text-sm text-slate-600 leading-relaxed">
                All tuition fees must be settled in full to access final marks
                and certification. Accounts with a status of{" "}
                <span className="font-bold text-rose-600">
                  CRITICAL OVERDUE
                </span>{" "}
                will have their results masked automatically until the balance
                is cleared. Payments are processed in real-time.
              </p>
            </CardContent>
          </Card>
        </div>
